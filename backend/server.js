...
app.post("/measurements", checkApiKey, async (req, res) => {
  // checkApiKey ajetaan ENNEN varsinaista käsittelijää
  try {
    // Aloitetaan virheenkäsittelylohko
    ...
    if (!deviceId || !status || !measuredAt) {
      ...
      return res.status(400).json({
        error: "Puuttuvia kenttiä: deviceId, status ja measuredAt vaaditaan",
      });
    }
    ...
    if (!saved) {
      ...
      return res
        .status(200)
        .json({ message: "Mittaus oli jo tallennettu (duplikaatti ohitettu)" });
    }
