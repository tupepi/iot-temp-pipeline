...
async function saveMeasurement({ deviceId, temperature, status, measuredAt }) { // Puretaan mittausolion kentät parametreiksi
  ...
  for (const key of cache.keys()) { // Käydään läpi kaikki välimuistiavaimet
...
async function getRecentMeasurements(deviceId, hours = 24) { // hours oletuksena 24 tuntia
  ...
  if (cached && now - cached.timestamp < CACHE_TTL) { // Tarkistetaan onko välimuisti vielä voimassa
...
async function getDevice(deviceId) { // Haetaan laite annetulla tunnisteella
  const result = await pool.query(
    // Suoritetaan SQL-kysely
...
async function getWeatherForecasts(pastHours = 24, futureHours = 12) { // Määritellään oletusarvot parametreille
  const result = await pool.query(
    // Suoritetaan SQL-kysely
...
async function saveWeatherForecast(forecastTime, temperature, symbolCode) { // Parametrit yhdelle ennustepisteelle
  await pool.query(
    // Suoritetaan SQL-kysely
...
async function closePool() { // Ei ota parametreja
  await pool.end(); // Suljetaan kaikki poolin yhteydet
} // Funktion loppu

module.exports = { // Viedään moduulin funktiot
